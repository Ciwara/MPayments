...
def ensure_default_owner(cursor):
    ...
    try:
        ...
        if not owner:
            ...
            if existing_admin:
                admin_id, is_logged = existing_admin
                if not is_logged:
                    # Mettre à jour l'Owner admin existant
            ...
            # Créer un nouvel Owner par défaut
            try:
                ...
                # Tentative avec un nom d'utilisateur unique
                import random
        ...
        print(f"❌ Erreur lors de la récupération/création de l'Owner: {str(e)}")
        return None
